...
class SPS(BaseModule):
    ...
    def forward(self, x):
        ...
        x = x.transpose(-1, -2)  # TB, -1, H//16, w//16
...
class SDT_Block_s(nn.Module):
    def __init__(self, embed_dim=384, num_heads=12, step=4, mlp_ratio=4.,mlp_drop=0., drop_path=0.2):
        super().__init__()
...
class SDTV1(BaseModule):
    def __init__(self, step=4,img_size=224, patch_size=16, in_channels=3, num_classes=1000,embed_dim=512,
                 num_heads=8, mlp_ratio=4,mlp_drop=0., depths=4, drop_path_rate = 0.2):
        super().__init__(step=step, encode_type='direct',layer_by_layer=True)
...
@register_model
def TIMv2_bl_sdt_img(pretrained=False,**kwargs):
    model = SDTV1(
