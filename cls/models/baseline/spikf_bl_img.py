...
class MLP(BaseModule):
    def __init__(self, in_features, step=4, mlp_ratio = 4.0, out_features=None,mlp_drop=0., drop_path=0.2):
...
class SSA(BaseModule):
    ...
    def forward(self, x):
        ...
        x = attn @ v
        x = x.transpose(-1, -2).reshape(TB, C, N).contiguous()
        x = self.attn_lif(x) # TB C N
        x = self.proj_lif(self.proj_bn(self.proj_linear(x)))

        x = x.reshape(TB, C, H, W).contiguous()
        return x # TB C H W

# Spikformer block
class Block(nn.Module):
    def __init__(self, embed_dim=512, num_heads=8, step=4, mlp_ratio=4. ,attn_scale=0.125,mlp_drop=0., drop_path=0.0):
        super().__init__()
        ...
        self.mlp = MLP(step=step,in_features=embed_dim,mlp_ratio=mlp_ratio,out_features=embed_dim,
                       mlp_drop=mlp_drop, drop_path=drop_path)
...
class Spikformer(BaseModule):
    def __init__(self,
                 step=4, img_size=224, patch_size=224, in_channels=3, num_classes=1000, attn_scale=0.125,
                 embed_dim=512, num_heads=8, mlp_ratio=4,depths=8, drop_path_rate = 0.2,
                 **kwargs):
        super().__init__(step=step, encode_type='direct',layer_by_layer=True)
...
@register_model
def TIMv2_bl_spikf_img(pretrained=False,**kwargs):
    model = Spikformer(
