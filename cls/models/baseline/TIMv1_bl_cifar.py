...
class TIMv1(BaseModule):
    def __init__(self,in_channels, step = 4, TIM_alpha = 0.5):
        super().__init__(step=step, encode_type='direct', layer_by_layer=True)
        self.T = step

        # in_channels should be N
        # 相当于是在不同的patch间交互
...
class SSA(BaseModule):
    def __init__(self,embed_dim, step=4, num_heads=12,attn_scale=0.125):
        ...
        self.TIMv1 = TIMv1(in_channels=64, step=4)
...
@register_model
def TIMv2_bl_TIMv1_cifar(pretrained=False,**kwargs):
    model = Spikformer(
