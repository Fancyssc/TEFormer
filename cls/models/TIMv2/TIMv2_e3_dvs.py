...
class TIMv2(BaseModule):
    def __init__(self, step = 4):
        ...
        alpha_init = 0.75
        theta_init = torch.logit((torch.tensor(alpha_init) - 0.5) / 0.5) # 始终保证
...
class Token_QK_Attention(BaseModule):
    def __init__(self, embed_dim, step=4, num_heads=12):
        ...
        self.proj_lif = LIFNode(step=self.step, tau=2.0,threshold=0.5, mem_detach=False)
    ...
    def forward(self, x):
        ...
        v_conv_out = self.v_bn(v_conv_out).reshape(TB, C, H, W).contiguous()  # TB C H W
        v = self.TIMv2(v_conv_out)
        v = self.v_lif(v).flatten(-2, -1)  # TB C N
...
class T_MLP(BaseModule):
    def __init__(self, in_features, step=4, mlp_ratio = 2.0, out_features=None):
        ...
        self.gate_lif = LIFNode(step=1, tau=2.0, threshold=0.5, mem_detach=False)
...
class PatchInit(BaseModule):
    def __init__(self, step=4, img_h=32, img_w=32, patch_size=4, in_channels=3, embed_dim=384):
        ...
        self.proj1_conv = nn.Conv2d(embed_dim // 8, embed_dim // 4, kernel_size=3, stride=1, padding=1, bias=False)
        self.proj1_bn = nn.BatchNorm2d(embed_dim // 4)
        self.proj1_lif = LIFNode(step=self.step, tau=2.0,threshold=1.0, mem_detach=False)

        self.proj2_conv = nn.Conv2d(embed_dim // 4, embed_dim // 2, kernel_size=3, stride=1, padding=1, bias=False)
        self.proj2_bn = nn.BatchNorm2d(embed_dim // 2)
        ...
        # 调整了stride
        self.proj_res_conv = nn.Conv2d(embed_dim // 4 , embed_dim, kernel_size=1, stride=2, padding=0, bias=False)
...
class PatchEmbedding(BaseModule):
    def __init__(self, step=4, encode_type='direct', img_h=32, img_w=32, patch_size=4, in_channels=3, embed_dim=384):
        ...
        self.proj4_conv = nn.Conv2d(embed_dim // 2, embed_dim, kernel_size=3, stride=1, padding=1, bias=False)
        self.proj4_bn = nn.BatchNorm2d(embed_dim)
        self.proj4_lif = LIFNode(step=self.step, tau=2.0,threshold=1.0, mem_detach=False)

        self.proj5_conv = nn.Conv2d(embed_dim, embed_dim, kernel_size=3, stride=1, padding=1, bias=False)
        self.proj5_bn = nn.BatchNorm2d(embed_dim)
        self.proj5_maxpool = torch.nn.MaxPool2d(kernel_size=3, stride=2, padding=1, dilation=1, ceil_mode=False)
        self.proj5_lif = LIFNode(step=self.step, tau=2.0,threshold=1.0, mem_detach=False)
...
class QKFormer(BaseModule):
    def __init__(self,
                 ...
                 embed_dim=256, num_heads=16, mlp_ratio=4, scale=0.125, depths=4, ):
        super().__init__(step=step, encode_type='direct', layer_by_layer=True)
        ...
        patch_embed1 = PatchInit(step=step, img_h=img_size, img_w=img_size, patch_size=patch_size,
                                 in_channels=in_channels,embed_dim=embed_dim//2)
        ...
        patch_embed2 = PatchEmbedding(step=step, img_h=img_size, img_w=img_size, patch_size=patch_size,
                                      in_channels=in_channels,embed_dim=embed_dim//1)

        stage2 = nn.ModuleList([TokenSpikingTransformer(
                                 embed_dim=embed_dim//1, step=step, num_heads=num_heads, mlp_ratio=mlp_ratio,
)
                            for j in range(1)])
...
@register_model
def TIMv2_e3_dvs(pretrained=False, **kwargs):
    model = QKFormer(
