...
# for discussion
class TIMv2_fixed_a(BaseModule):
...
class TIMv2_n(BaseModule):
    ...
    def forward(self, x):
        ...
        x = torch.flip(K @ torch.flip(x, dims=[0]), dims=[0])  # TIMv2 (reverse direction)
...
class Token_QK_Attention(BaseModule):
    def __init__(self, embed_dim, step=4, num_heads=12):
        ...
        self.proj_lif = LIFNode(step=self.step, tau=2.0,threshold=0.5, mem_detach=False) # 也算attn lif了
    ...
    def forward(self, x):
        ...
        x = torch.mul(attn, k) # TB num_heads C//num_heads N

        x = x.flatten(1, 2) # TB C N
        #
...
@register_model
def TIMv2_e3_cifar(pretrained=False, **kwargs):
    model = QKFormer(
